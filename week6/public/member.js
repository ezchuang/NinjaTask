...
function generateMsg(targetId, max, msgs, mem_id){
    ...
    for (data of msgs){
        ...
        if (id != mem_id){
            ...
            // 插入表單元素(隱藏的)，提送用
            let invalidItem_1 = document.createElement("input")
            childTag.appendChild(invalidItem_1)
            invalidItem_1.setAttribute("type", "hidden")
            invalidItem_1.setAttribute("name", "id")
            invalidItem_1.setAttribute("value", id)
            let invalidItem_2 = document.createElement("input")
            childTag.appendChild(invalidItem_2)
            invalidItem_2.setAttribute("type", "hidden")
            invalidItem_2.setAttribute("name", "msg_id")
            invalidItem_2.setAttribute("value", msg_id)
        }
